...
--- @class _99sync.window.Module
--- @field active_windows _99sync.window.Window[]
local M = {
  active_windows = {},
}
...
--- @class _99sync.window.Window
--- @field config _99sync.window.Config
--- @field win_id number
--- @field buf_id number
--- @field type "capture_input" | "status"

--- @class _99sync.window.SplitWindow
--- @field win number
--- @field buffer number
...
--- @param win _99sync.window.Window
...
--- @param config _99sync.window.Config
--- @param title string?
...
--- @param window _99sync.window.Window
...
function M.display_error(error_text)
  local window =
    create_floating_window(create_window_top_config(), " 99sync Error ", false)
...
function M.display_cancellation_message(text)
  local config = create_transparent_top_right_config(100, " 99sync Cancelled ")
  local window = create_floating_window(config, " 99sync Cancelled ", false)
...
function M.display_full_screen_message(lines)
  ...
  M.clear_active_popups()
  local window =
    create_floating_window(create_window_full_screen(), " 99sync ", true)
...
function M.create_centered_window()
  ...
  local window = create_floating_window(config, " 99sync ", true)
...
local function set_defaul_win_options(win, name)
  ...
  vim.bo[win.buf_id].filetype = "99sync"
...
--- @alias _99sync.window.KeyMap table<string, string>
--- @class _99sync.window.CaptureInputOpts
--- @field cb fun(success: boolean, result: string): nil
--- @field on_load? fun(): nil
--- @field content? string[]
--- @field rules? _99sync.Agents.Rules
--- @field keymap? _99sync.window.KeyMap

--- @param keymap _99sync.window.KeyMap
--- @param width number
--- @return string[]
...
--- @param name string
--- @param opts _99sync.window.CaptureInputOpts
function M.capture_input(name, opts)
  M.clear_active_popups()

  local config = create_centered_window()
  local win =
    create_floating_window(config, string.format(" 99sync %s ", name), true)
...
function M.status_window()
  M.clear_active_popups()
  local config = create_transparent_top_right_config(100, " 99sync - Status ")
  local window = create_floating_window(config, " 99sync - Status ", false)
  window.type = "status"
  return window
end
...
--- @class _99sync.window.SplitWindowOpts
...
---@param opts _99sync.window.SplitWindowOpts | nil
--- @return _99sync.window.SplitWindow
