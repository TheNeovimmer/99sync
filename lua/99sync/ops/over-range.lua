local RequestStatus = require("99sync.ops.request_status")
local Mark = require("99sync.ops.marks")
...
local function over_range(context, opts)
  ...
  local display_ai_status = context._99sync.ai_stdout_rows > 1
  local top_status = RequestStatus.new(
    250,
    context._99sync.ai_stdout_rows or 1,
    "Implementing",
    top_mark
  )
  ...
  local system_cmd = context._99sync.prompts.prompts.visual_selection(range)
