...
local function run(provider, operation, status, prompt)
  _99sync[operation]({ additional_prompt = prompt })
...
describe("tracking", function()
  it("serializes requests based on configured counts", function()
    ...
    _99sync.setup(test_utils.get_test_setup_options({
    ...
    local serialized = _99sync.__get_state().tracking:serialize()
    local actual_counts = {
      search = 0,
      vibe = 0,
      tutorial = 0,
  ...
  it("successful returns things in reverse order", function()
    ...
    local successful = _99sync.__get_state().tracking:successful()
    eq(3, #successful)
