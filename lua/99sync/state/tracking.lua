...
--- @class _99sync.State.Tracking.Serialized
--- @field requests _99sync.Prompt.Serialized[]

--- @class _99sync.State.Tracking.Config.Options.Counts
--- @field vibe number | nil
--- @field search number | nil
--- @field tutorial number | nil
--- @field visual number | nil
---
--- @class _99sync.State.Tracking.Config.Options
--- @field serialize_counts _99sync.State.Tracking.Config.Options.Counts | nil

--- @class _99sync.State.Tracking.Config
--- @field serialize_counts table<_99sync.Prompt.Operation, number>

--- @class _99sync.State.Tracking
--- @docs base
--- @field history _99sync.Prompt[]
--- @field id_to_request table<number, _99sync.Prompt>
--- @field setup fun(opts: _99sync.State.Tracking.Config.Options): nil
local Tracking = {}
Tracking.__index = Tracking

--- @param _99sync _99sync.State
--- @param previous_state _99sync.State.Tracking.Serialized | nil
--- @return _99sync.State.Tracking
function Tracking.new(_99sync, previous_state)
  local tracking = setmetatable({}, Tracking) --[[ @as _99sync.State.Tracking]]
  ...
  for _, d in ipairs(previous_state.requests or {}) do
    local prompt = Prompt.deserialize(_99sync, d)
...
--- @return _99sync.State.Tracking.Serialized
function Tracking:serialize()
  local sc = Tracking.__config.serialize_count

  --- @type table<_99sync.Prompt.Operation, _99sync.Prompt[]>
  local all_requests = {}
  for _, r in ipairs(self.history) do
...
--- @param opts _99sync.State.Tracking.Config.Options
